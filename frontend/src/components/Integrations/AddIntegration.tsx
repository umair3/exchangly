...
import { useAddIntegration } from "../../services/Api/Integrations/hooks";
import {
  CircularLoader,
  CustomButton,
...
const AddIntegration: React.FC<IAddIntegrationProps> = ({ closeModal }) => {
  const classes = useStyles();

  const { addIntegration, isLoading } = useAddIntegration(closeModal);

  const { handleSubmit, values, handleChange, handleBlur, errors, touched } =
    useAddEditIntegrationForm({
      handleValues: ({ port, ...values }) =>
        addIntegration({ ...values, port: parseInt(port) }),
    });
  ...
  return (
    <React.Fragment>
      ...
      <form className={classes.form}>
        <h4 className={classes.header}>Create new integration</h4>
